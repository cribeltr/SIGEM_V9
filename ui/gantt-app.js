...
(function () {
  ...
  // ---- sub-formulario: nuevo evento ----
  function paintEvento(e) {
    drawerMode = 'evento';
    subForm(e, 'Nuevo evento',
      '<div class="fld"><label>Tipo</label><select id="eTipo">' + opt(TIPOS_EV, 'Solicitud de trabajo') + '</select></div>' +
      '<div class="grid-2"><div class="fld"><label>Fecha</label><input type="date" id="eFecha" value="' + esc(H.hoyLocal()) + '"></div>' +
        '<div class="fld"><label>Ejecutor</label><select id="eEjec">' + opt([['', '—']].concat(EJEC.map(function (x) { return [x, x]; })), '') + '</select></div></div>' +
      '<div class="fld"><label>N° Folio / Informe (opcional)</label><input id="eFolio" placeholder="folio, OC, cotización…"></div>' +
      '<div class="fld"><label>Observación</label><textarea id="eObs" placeholder="Detalle del evento…"></textarea></div>' +
      '<div class="notice">Los efectos se aplican según el tipo: Solicitud abre ciclo y deja el equipo no operativo; Reparación/Recepción operativa cierra el ciclo; etc.</div>',
      function () {
        var d = { inv: e.inv, tipo: el('#eTipo').value, fecha: el('#eFecha').value, ejecutor: el('#eEjec').value, folio: el('#eFolio').value, obs: el('#eObs').value };
        ...
        toast('Evento creado · ' + d.tipo); paintEquipo(H.findEquipo(e.inv) || e);
      });
  }
  ...
  function paintPendiente(e) {
    ...
    subForm(e, 'Nuevo pendiente',
      ...
      '<div class="grid-2"><div class="fld"><label>Responsable</label><select id="pEjec">' + opt([['', '—']].concat(EJEC.map(function (x) { return [x, x]; })), '') + '</select></div>' +
        '<div class="fld"><label>Compromiso (opcional)</label><input type="date" id="pComp"></div></div>',
      function () {
        var d = { inv: e.inv, tipo: el('#pTipo').value, desc: el('#pDesc').value, ejecutor: el('#pEjec').value || null, fechaComp: el('#pComp').value || null };
  ...
  // armazón común de sub-formularios
  function subForm(e, title, bodyHtml, onSave) {
    drawer.innerHTML =
      '<div class="d-head"><button class="d-close" data-x>✕</button><div class="d-fam">' + esc(e.fam || '') + '</div>' +
      '<div class="d-name">' + esc(title) + '</div><div class="d-mm">' + esc(e.equipo || '') + ' · ' + esc(e.inv) + '</div></div>' +
      '<div class="d-body"><button class="dback" data-back>← Volver al equipo</button>' + bodyHtml + '</div>' +
      '<div class="d-foot"><button class="savebtn" data-save>Guardar</button><button class="cancel" data-back>Cancelar</button></div>';
    var back = function () { paintEquipo(H.findEquipo(e.inv) || e); };
    drawer.querySelector('[data-x]').onclick = closeDrawer;
    var backs = drawer.querySelectorAll('[data-back]'); for (var b = 0; b < backs.length; b++) backs[b].onclick = back;
    drawer.querySelector('[data-save]').onclick = onSave;
  }
