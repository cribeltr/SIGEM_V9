...
(function () {
  ...
  function showScreen(name) {
    ...
    if (name === 'gantt') { g.style.display = ''; alt.style.display = 'none'; if (tbar) tbar.style.display = ''; render(); }
      ...
      g.style.display = 'none'; alt.style.display = ''; if (tbar) tbar.style.display = 'none';
      if (name === 'tablero') renderTablero();
      else if (name === 'cumplimiento') renderCumplimiento();
      else if (name === 'contactos') renderContactos();
      else if (name === 'auditoria') renderAuditoria();
      else renderMiDia();
    }
  }
  function nombreCorto(n) { if (!n) return ''; var p = String(n).trim().split(/\s+/); return p.length > 1 ? (p[0] + ' ' + p[p.length - 1]) : p[0]; }
  function renderMiDia() {
    var alt = el('#altScreen'), S = H.getState(), hoy = H.hoyLocal();
    var pend = (S.pendientes || []).filter(function (p) { return !p.anulado && p.estado !== 'cerrado'; });
    var baseDe = function (p) { var fs = (p.seguimientos || []).map(function (s) { return s.fecha; }).filter(Boolean).sort(); var ult = fs[fs.length - 1]; return (ult && ult > (p.fechaCrea || '')) ? ult : (p.fechaCrea || hoy); };
    var lista = pend.map(function (p) { var dias = H.diasEntreFechas(baseDe(p), hoy); return { p: p, dias: dias, recordar: dias >= 3 }; }).sort(function (a, b) { return (b.recordar - a.recordar) || (b.dias - a.dias) || ((a.p.fechaComp || '9999').localeCompare(b.p.fechaComp || '9999')); });
    var porRecordar = lista.filter(function (x) { return x.recordar; }).length;
    var sinResp = pend.filter(function (p) { return !p.ejecutor; });
    var vivos = S.equipos.filter(function (e) { return e.estado !== 'baja'; });
    var noop = S.equipos.filter(function (e) { return e.estado === 'no_operativo'; }).length;
    var stc = S.equipos.filter(function (e) { return e.estado === 'en_servicio_tecnico'; }).length;
    var d30 = S.equipos.filter(function (e) { return ['no_operativo', 'en_servicio_tecnico'].indexOf(e.estado) >= 0 && H.diasEnEstado(e) > 30; }).length;
    var mpMes = vivos.filter(function (e) { return H.mpProgramadaEnMes(e, MES[NOW]) && H.mpEstadoMes(e, YEAR, NOW) === 'pendiente'; }).length;
    var mpAtr = vivos.filter(function (e) { for (var m = 0; m < NOW; m++) { if (H.mpProgramadaEnMes(e, MES[m]) && H.mpEstadoMes(e, YEAR, m) === 'pendiente') return true; } return false; }).length;
    var borr = S.eventos.filter(function (e) { return !e.anulado && e.tipo === 'Mantención preventiva' && e.oficial !== 'Sí'; }).length;
    var sinProg = vivos.filter(function (e) { return H.sinProgramacionMP(e); }).length;
    var conf = (S.conflictos || []).filter(function (c) { return c.estado === 'pendiente' || c.estado === 'pospuesto'; }).length;
    var hh = new Date().getHours(), saludo = hh < 12 ? 'Buenos días' : hh < 20 ? 'Buenas tardes' : 'Buenas noches';
    var fechaLarga = hoy; try { fechaLarga = new Date(hoy + 'T00:00:00').toLocaleDateString('es-CL', { weekday: 'long', day: 'numeric', month: 'long' }); } catch (_) {}
    var al = function (n, lab, cls, act) { return '<button class="mday-alert ' + (cls || '') + '" data-act="' + (act || '') + '"' + (n ? '' : ' disabled') + '><b>' + n + '</b><span>' + lab + '</span></button>'; };
    var alerts = al(noop, 'No operativos', 'no', 'estado:no_operativo') + al(stc, 'En serv. técnico', 'warn', 'estado:en_servicio_tecnico') + al(d30, '> 30 días', 'no', '') + al(mpMes, 'MP del mes', 'warn', 'cmp') + al(mpAtr, 'MP atrasadas', 'warn', 'cmp') + al(borr, 'Borradores', '', '') + al(sinProg, 'Sin programación', '', '') + al(conf, 'Conflictos', 'warn', 'conf');
    var tasks = '';
    if (sinResp.length) tasks += '<div class="mday-task"><div style="flex:1"><div class="mt-t">' + sinResp.length + ' pendiente(s) sin delegar</div><div class="mt-s">Asígnales un responsable abajo para no perderles el seguimiento.</div></div></div>';
    if (conf) tasks += '<div class="mday-task" data-go="conf"><div style="flex:1"><div class="mt-t">' + conf + ' conflicto(s) con el maestro</div><div class="mt-s">Revísalos en Conciliación.</div></div><button class="mini" data-go="conf">Revisar</button></div>';
    var card = function (x) {
      var p = x.p, rec = x.recordar;
      return '<div class="mday-card' + (rec ? ' rec' : '') + '" data-inv="' + esc(p.inv || '') + '"><div class="mc-main">' +
        (rec ? '<span class="mc-badge">' + (p.ejecutor ? 'Recuérdale a ' + esc(nombreCorto(p.ejecutor)) : 'Sin asignar — delégalo') + '</span>' : '') +
        '<div class="mc-title">' + esc(TIPO_PEND[p.tipo] || p.tipo) + (p.inv ? ' <span class="mono" style="color:var(--faint)">' + esc(p.inv) + '</span>' : '') + '</div>' +
        (p.desc ? '<div class="mc-desc">' + esc(p.desc) + '</div>' : '') +
        '<div class="mc-meta">' + (p.ejecutor ? 'Delegado a ' + esc(nombreCorto(p.ejecutor)) : 'Sin asignar') + ' · ' + (x.dias === 0 ? 'hoy' : 'hace ' + x.dias + ' día(s)') + (p.fechaComp ? ' · compromiso ' + esc(H.fmtFecha(p.fechaComp)) : '') + '</div></div>' +
        '<div class="mc-acts"><button class="mini" data-pa="solicitar" data-id="' + p.id + '">Solicitar</button>' +
        '<select class="mini mday-deleg" data-id="' + p.id + '">' + opt([['', 'Delegar…']].concat(EJEC.map(function (e) { return [e, nombreCorto(e)]; })), p.ejecutor || '') + '</select>' +
        '<button class="mini" data-pa="resolver" data-id="' + p.id + '">Resolver</button></div></div>';
    };
    alt.innerHTML = '<div class="mday"><div class="mday-hi"><h2>' + saludo + '</h2><span>' + esc(fechaLarga) + '</span></div>' +
      '<div class="mday-alerts">' + alerts + '</div>' + tasks +
      '<div class="scr-hd" style="margin-top:6px"><h2 style="font-size:16px">Pendientes priorizados</h2><span style="color:var(--muted);font-size:12px">' + pend.length + ' activo(s)' + (porRecordar ? ' · ' + porRecordar + ' por recordar' : '') + '</span></div>' +
      '<div class="mday-list">' + (lista.length ? lista.slice(0, 40).map(card).join('') : '<div class="sec-empty">✓ Sin pendientes activos.</div>') + '</div></div>';
    alt.querySelector('.mday-alerts').onclick = function (e) { var b = e.target.closest('[data-act]'); if (!b) return; var a = b.getAttribute('data-act'); if (!a) return; if (a.indexOf('estado:') === 0) { filt.estado = a.split(':')[1]; var fe = el('#fEstado'); if (fe) fe.value = filt.estado; showScreen('gantt'); } else if (a === 'conf') paintConflictos(); else if (a === 'cmp') { cmpMode = 'servicio'; showScreen('cumplimiento'); } };
    var list = alt.querySelector('.mday-list');
    list.onclick = function (e) {
      if (e.target.closest('.mc-acts')) { var btn = e.target.closest('[data-pa]'); if (btn) { e.stopPropagation(); var p = findPend(btn.getAttribute('data-id')); if (!p) return; var a = btn.getAttribute('data-pa'); if (a === 'solicitar') { H.agregarSeguimiento(p, 'Solicitado / recordado a ' + (p.ejecutor || '—')); toast('Registrado'); } else if (a === 'resolver') { H.cerrarPendiente(p, ''); } renderMiDia(); } return; }
      var card2 = e.target.closest('[data-inv]'); if (card2) { var eq = H.findEquipo(card2.getAttribute('data-inv')); if (eq) paintEquipo(eq); }
    };
    list.onchange = function (e) { var s = e.target.closest('.mday-deleg'); if (!s) return; var p = findPend(s.getAttribute('data-id')); if (p) { H.actualizarPendiente(p, { ejecutor: s.value || null }); toast('Delegado'); renderMiDia(); } };
    var tk = alt.querySelector('.mday-task[data-go="conf"]'); if (tk) tk.onclick = function () { paintConflictos(); };
  }
  ...
  /* ----- Contactos del servicio (editor) ----- */
  function renderContactos() {
    var alt = el('#altScreen'), servicios = uniqueServicios(), list = H.getContactos();
    var cargos = (H.CARGOS_CONTACTO || []);
    var rows = list.map(function (c) {
      return '<tr data-id="' + c.id + '">' +
        '<td class="l"><select class="c-f" data-k="servicio"><option value="">— todos —</option>' + servicios.map(function (s) { return '<option' + (s === c.servicio ? ' selected' : '') + '>' + esc(s) + '</option>'; }).join('') + '</select></td>' +
        '<td class="l"><input class="c-f" data-k="cargo" list="cargosDL" value="' + esc(c.cargo || '') + '"></td>' +
        '<td class="l"><input class="c-f" data-k="nombre" value="' + esc(c.nombre || '') + '"></td>' +
        '<td class="l"><input class="c-f" data-k="apellido" value="' + esc(c.apellido || '') + '"></td>' +
        '<td class="l"><input class="c-f" data-k="anexo" value="' + esc(c.anexo || '') + '"></td>' +
        '<td class="l"><input class="c-f" data-k="correo" type="email" value="' + esc(c.correo || '') + '"></td>' +
        '<td><button class="mini danger" data-del="' + c.id + '">✕</button></td></tr>';
    }).join('');
    alt.innerHTML = '<div class="scr-hd"><h2>Contactos del servicio</h2><div style="flex:1"></div><button class="tbtn" id="cAdd">+ Agregar contacto</button></div>' +
      '<div class="notice">Asigna un servicio para que el contacto aparezca en los equipos de ese servicio. "— todos —" = contacto general.</div>' +
      '<datalist id="cargosDL">' + cargos.map(function (c) { return '<option value="' + esc(c) + '">'; }).join('') + '</datalist>' +
      '<div style="overflow:auto;max-width:100%"><table class="cmp-table"><thead><tr><th class="l">Servicio</th><th class="l">Cargo</th><th class="l">Nombre</th><th class="l">Apellido</th><th class="l">Anexo</th><th class="l">Correo</th><th></th></tr></thead><tbody id="cBody">' + (rows || '') + '</tbody></table></div>';
    el('#cAdd').onclick = function () { H.agregarContacto({ cargo: '' }); renderContactos(); };
    var body = el('#cBody');
    body.onchange = function (e) { var f = e.target.closest('.c-f'); if (!f) return; var id = +f.closest('tr').getAttribute('data-id'); var o = {}; o[f.getAttribute('data-k')] = f.value; H.actualizarContacto(id, o); };
    body.onclick = function (e) { var d = e.target.closest('[data-del]'); if (!d) return; if (window.confirm('¿Eliminar este contacto?')) { H.eliminarContacto(+d.getAttribute('data-del')); renderContactos(); } };
  }

  /* ----- Auditoría · historial de cambios ----- */
  function renderAuditoria() {
    var alt = el('#altScreen');
    var aud = (H.getState().audit || []).slice().sort(function (a, b) { return (b.ts || '').localeCompare(a.ts || '') || (b.id - a.id); }).slice(0, 500);
    var fmt = function (ts) { try { return new Date(ts).toLocaleString('es-CL'); } catch (_) { return ts || ''; } };
    var v = function (x) { return x == null ? '—' : typeof x === 'object' ? JSON.stringify(x).slice(0, 60) : String(x); };
    var rows = aud.length ? aud.map(function (a) {
      return '<tr><td class="l mono" style="white-space:nowrap">' + esc(fmt(a.ts)) + '</td><td class="l">' + esc(a.entidad) + ' ' + esc(a.idEnt) + '</td><td class="l">' + esc(a.campo) + '</td><td class="l" style="color:var(--faint)">' + esc(v(a.valorAnterior)) + '</td><td class="l">' + esc(v(a.valorNuevo)) + '</td><td class="l">' + esc(a.usuario || '') + '</td></tr>';
    }).join('') : '<tr><td colspan="6" style="color:var(--faint);padding:14px;text-align:center">Sin cambios registrados.</td></tr>';
    alt.innerHTML = '<div class="scr-hd"><h2>Auditoría · historial de cambios</h2><div style="flex:1"></div><span style="color:var(--muted);font-size:12px">' + aud.length + ' cambios (recientes)</span></div>' +
      ...
      '<header class="top"><div class="top-row">' +
        ...
        '<div id="moreWrap" style="position:relative">' +
          ...
          '<div class="more-menu" id="moreMenu">' +
            '<button class="more-item" data-screen="midia">Mi día</button>' +
            ...
            '<button class="more-item" data-screen="contactos">Contactos</button>' +
            '<button class="more-item" data-screen="auditoria">Auditoría</button>' +
